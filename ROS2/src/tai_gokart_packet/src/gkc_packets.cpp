...
{
  auto buffer = std::make_unique<GkcBuffer>(payload_size + 6, 0);
  *(buffer->begin()) = RawGkcPacket::START_BYTE;
  GkcPacketUtils::write_to_buffer(buffer->begin(), payload_size);
  std::copy(payload.begin(), payload.end(), buffer->begin() + 3);
  GkcPacketUtils::write_to_buffer(buffer->begin() + payload_size + 3, checksum);
  buffer->back() = RawGkcPacket::END_BYTE;
  return buffer;
}
