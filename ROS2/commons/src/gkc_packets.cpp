...
RawGkcPacket::RawGkcPacket()
    : payload_size(0), payload(GkcBuffer()), checksum(0) {}
...
RawGkcPacket::RawGkcPacket(const GkcPacket& packet) { *this = packet; }
...
RawGkcPacket::SharedPtr Handshake1GkcPacket::encode() {
...
void Handshake1GkcPacket::decode(const RawGkcPacket& raw) {
  GkcPacketUtils::read_from_buffer<uint32_t>(raw.payload.begin() + 1,
                                             seq_number);
}

RawGkcPacket::SharedPtr Handshake2GkcPacket::encode() {
  GkcBuffer payload = GkcBuffer(5);
  payload[0] = FIRST_BYTE;
  GkcPacketUtils::write_to_buffer<uint32_t>(payload.begin() + 1, seq_number);
  return std::make_unique<RawGkcPacket>(payload);
...
RawGkcPacket::SharedPtr GetFirmwareVersionGkcPacket::encode() {
...
RawGkcPacket::SharedPtr FirmwareVersionGkcPacket::encode() {
...
RawGkcPacket::SharedPtr ResetImu::encode() {
  GkcBuffer payload = GkcBuffer(5);
  payload[0] = FIRST_BYTE;
  GkcPacketUtils::write_to_buffer<uint32_t>(payload.begin() + 1, seq_number);
  return std::make_unique<RawGkcPacket>(payload);
...
void ResetImu::decode(const RawGkcPacket& raw) {
  GkcPacketUtils::read_from_buffer<uint32_t>(raw.payload.begin() + 1,
                                             seq_number);
}
